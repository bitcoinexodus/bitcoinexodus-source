...
const char * const BITCOINEXODUS_CONF_FILENAME = "bitcoinexodus.conf";
const char * const BITCOINEXODUS_PID_FILENAME = "bitcoinexodusd.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "bitcoinexodusd -foo=bar
...
#else
    const char* pszModule = "bitcoinexodus";
#endif
    if (pex)
        return strprintf(
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\BitcoinExodus
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\BitcoinExodus
    // Mac: ~/Library/Application Support/BitcoinExodus
    // Unix: ~/.bitcoinexodus
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BitcoinExodus";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/BitcoinExodus";
#else
    // Unix
    return pathRet / ".bitcoinexodus";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", BITCOINEXODUS_CONF_FILENAME);
...
fs::path GetPidFile()
{
    return AbsPathForConfigVal(fs::path(gArgs.GetArg("-pid", BITCOINEXODUS_PID_FILENAME)));
}
