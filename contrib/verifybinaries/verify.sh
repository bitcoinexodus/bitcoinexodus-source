...
###   bitcoinexoduscore.org and bitcoinexodus.org and compares them.
...
WORKINGDIR="/tmp/bitcoinexodus_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://bitcoinexoduscore.org"
HOST2="https://bitcoinexodus.org"
BASEDIR="/bin/"
VERSIONPREFIX="bitcoinexodus-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'bitcoinexodus-',
...
if [ $? -ne 0 ]; then
   echo "bitcoinexodus.org failed to provide signature file, but bitcoinexoduscore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "bitcoinexodus.org and bitcoinexoduscore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the BitcoinExodus Core binary release signing key installed?"
   fi
